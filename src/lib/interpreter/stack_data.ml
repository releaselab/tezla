...
let rec typ_from_adt_typ =
  let open Adt.Typ in
  function
  | T_address -> E_T Address_t
  | T_key -> E_T Key_t
  | T_unit -> E_T Unit_t
...
let rec from_adt_data : type a. a typ -> Adt.data -> a t =
 fun t d ->
  ...
  | (Big_map_t _ | Contract_t _ | Operation_t), _ -> raise Non_pushable_type
  | ( Address_t,
      ( D_unit | D_none | D_int _ | D_bytes _ | D_bool _
      | D_pair (_, _)
      | D_left _ | D_right _ | D_some _
      | D_elt (_, _)
      | D_list _ | D_instruction _ ) )
  | ( Bool_t,
...
and list_from_adt_data : type a. a typ -> data list -> a t_list =
 fun t -> function
  | [] -> L_nil
  | d :: tl ->
      let d' = from_adt_data t d in
...
and set_from_adt_data : type a. a typ -> data list -> a t_set =
 fun t -> function
  | [] -> Set_nil
  | d :: tl ->
      let d' = from_adt_data t d in
...
and map_from_adt_data : type k v. k typ -> v typ -> data list -> (k, v) t_map =
 fun t_k t_v d_l ->
  match d_l with
  | [] -> Map_nil
...
let rec equal : type a. a t -> a t -> bool =
 fun d_1 d_2 ->
  ...
  | SD_pair (d_1, d_2), SD_pair (d_3, d_4) ->
...
let rec cast : type a b. a typ -> b t -> a t =
 fun t d ->
  ...
  | Pair_t (t_1, t_2), SD_pair (d_1, d_2) -> SD_pair (cast t_1 d_1, cast t_2 d_2)
...
let rec to_string : type a. a t -> string = function
  ...
  | SD_timestamp (Timestamp n) | SD_int (Int n) | SD_nat (Nat n) ->
      Bigint.to_string n
  ...
  | SD_pair (d_1, d_2) -> [%string "(%{to_string d_1}, %{to_string d_2})"]
  | SD_operation (Operation o) -> Operation.to_string o
