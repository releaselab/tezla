...
let rec expr (s : int) ctx env =
  ...
  | E_div (v_1, v_2) ->
      ...
      let res =
        ...
        | SD_nat (Nat n_1), SD_nat (Nat n_2) -> (
            ...
            | Some (q, r) ->
                E
                  (SD_option (O_some (SD_pair (SD_nat (Nat q), SD_nat (Nat r)))))
            )
        | SD_nat (Nat n_1), SD_int (Int n_2)
        | SD_int (Int n_1), SD_nat (Nat n_2)
        | SD_int (Int n_1), SD_int (Int n_2) -> (
            ...
            | Some (q, r) ->
                E
                  (SD_option (O_some (SD_pair (SD_int (Int q), SD_nat (Nat r)))))
            )
        | SD_mutez (Mutez n_1), SD_nat (Nat n_2) -> (
            let n_1 = Bigint.of_int64 n_1 in
            match div n_1 n_2 with
            ...
            | Some (q, r) ->
                ...
                E
                  (SD_option
                     (O_some (SD_pair (SD_mutez (Mutez q), SD_mutez (Mutez r)))))
            )
        | SD_mutez (Mutez n_1), SD_mutez (Mutez n_2) -> (
            let n_1 = Bigint.of_int64 n_1 in
            let n_2 = Bigint.of_int64 n_2 in
  ...
  | E_size v -> (
      let (E d) = find v in
      let rec f_list : type a. Bigint.t -> a t_list -> Bigint.t =
       fun acc -> function
        | L_nil -> acc
        | L_cons (_, tl) -> f_list Bigint.(acc + one) tl
      in
      let rec f_set : type a. Bigint.t -> a t_set -> Bigint.t =
       fun acc -> function
        | Set_nil -> acc
        | Set_cons (_, tl) -> f_set Bigint.(acc + one) tl
      in
      let rec f_map : type a b. Bigint.t -> (a, b) t_map -> Bigint.t =
       fun acc -> function
        | Map_nil -> acc
